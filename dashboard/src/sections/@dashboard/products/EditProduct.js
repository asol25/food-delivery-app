...
const uploader = Uploader({
  apiKey: `${process.env.REACT_APP_UPLOADER_API_UPLOADER_KEY || 'public_FW25azqzzY2fME4S33UQQMhgyjTe'}`,
});
...
export default function EditProduct(props) {
  ...
  const { id, sale, title, thumbnail, cost, desc, setProducts, products, category, handleClose } = props;
  ...
  const handleSave = async () => {
    ...
    try {
      console.log('🚀 ~ file: EditProduct.js:82 ~ handleSave ~ sendData', sendData);
      const save = await apisProducts.updateProducts(sendData);
      const { data, status } = save;
      if (status === 200) {
        // eslint-disable-next-line no-return-assign
        setProducts(products.map((item) => (item.id === data.id ? (item = data) : item)));
        handleClose();
      }
    } catch (error) {
    ...
    <>
      <Stack flexDirection={'row'}>
        ...
        <List>
          <ListItem>
            <div>
              {files.length ? <UploadFileComponent files={files} /> : <UploadButtonComponent setFiles={setFiles} />}
            </div>
          </ListItem>
          <ListItem>
            <div>
